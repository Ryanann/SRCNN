...
# every image is cut into several sub images as training data
def prepare_data(_path):
    ...
    for i in range(nums):
        ...
        for j in range(Random_Crop):
            ...
            # SRCNN output's width = input's width - 12, so parts of the label image has to be cut out
            label[i * Random_Crop + j, 0, :, :] = hr_patch[conv_side: -conv_side, conv_side: -conv_side]
            # cv2.imshow("lr", lr_patch)
            # cv2.imshow("hr", hr_patch)
            # cv2.waitKey(0)
    print("data_shape_1: " + str(data.shape))
    print("label_shape_1: " + str(label.shape))
    return data, label
...
if __name__ == "__main__":
    ...
    _, _a = read_training_data("crop_train.h5")
